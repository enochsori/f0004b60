  ...
  return (
    <header className='h-[60px] p-2 dark:bg-bg-dark-dark bg-bg-light-dark rounded-t-2xl px-4 border-b dark:border-color-dark-grey border-color-light-grey flex justify-between relative'>
      ...
      <div className='hover:cursor-pointer' onClick={() => navigate('/')}>
        ...
        >
          <g stroke='none' strokeWidth='1' fill='none' fillRule='evenodd'>
            <g transform='translate(170.000000, 24.000000)'>
              <rect
                fill='var(--color-logo)'
                x='270'
                y='6'
      ...
      <div className='flex items-center gap-1 text-sm'>
        {currentPage === 'AF' && (
          <TbActivityHeartbeat
            className={`text-color-accent-second text-2xl`}
          />
        )}
        ...
        {currentPage !== 'AD' && currentPage !== 'AF' && (
          <MdSdStorage className='text-color-accent-second text-2xl' />
        )}

        <span className='dark:text-color-text-dark'>{currentPage}</span>
      </div>
    </header>
  );
}
